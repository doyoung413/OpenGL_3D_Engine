...
public:
	...
	bool IsKeyReleasedOnce(KEYBOARDKEYS keycode);

	bool IsMouseButtonPressed(MOUSEBUTTON button);
	bool IsMouseButtonPressOnce(MOUSEBUTTON button);
	bool IsMouseButtonReleasedOnce(MOUSEBUTTON button);

	glm::vec2 GetMousePosition();
	glm::vec2 GetMouseWheelMotion();
	glm::vec2 GetRelativeMouseMotion();

	void SetRelativeMouseMode(bool state);
	bool GetRelativeMouseMode();

private:
	void KeyDown(KEYBOARDKEYS keycode) { keyStates[keycode] = true; }
	void KeyUp(KEYBOARDKEYS keycode) { keyStates[keycode] = false; }
	void MouseButtonDown(MOUSEBUTTON button) { mouseButtonStates[button] = true; }
	void MouseButtonUp(MOUSEBUTTON button) { mouseButtonStates[button] = false; }
	...
	glm::vec2 mouseWheelMotion = glm::vec2(0.0f);
	glm::vec2 relativeMouseMotion = glm::vec2(0.0f);
};